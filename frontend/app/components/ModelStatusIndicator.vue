...
<template>
  <div class="flex items-center gap-2 text-sm">
    ...
    <span v-if="status === 'loading'" class="i-lucide-loader animate-spin w-4 h-4" />

    <!-- Ready state: green check -->
    <span v-else-if="modelLoaded" class="i-lucide-check-circle w-4 h-4 text-green-500" />

    <!-- Error state: red alert -->
    <span v-else class="i-lucide-alert-circle w-4 h-4 text-red-500" />
